...
| --------------------------------- | ------- | -------------------------------------------------------------- |
| `APP_ENV`                         | server  | `production` ใช้ `CIS_ENDPOINT_PROD`, ค่าอื่นใช้ `CIS_ENDPOINT_UAT` |
...
- [x] สเปค payload ของ CIS `SaveOtherSource` — ปรับที่ `toCisPayload()` ใน `src/lib/cis.ts`
